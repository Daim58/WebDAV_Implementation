import md5 from "md5";
import { ha1Compute } from "../newDependency/crypto";
import { DigestContext, RequestOptionsWithState, Response } from "../types";
...
export function generateDigestAuthHeader(options: RequestOptionsWithState, digest: DigestContext): string {
    ...
    for (const k in authValues) {
        if (authValues[k as keyof typeof authValues]) {
            if (k === "qop" || k === "nc" || k === "algorithm") {
                authHeader.push(`${k}=${authValues[k as keyof typeof authValues]}`);
            } else {
                authHeader.push(`${k}="${authValues[k as keyof typeof authValues]}"`);
            }
        }
...
export function parseDigestAuth(response: Response, _digest: DigestContext | any ): boolean {
